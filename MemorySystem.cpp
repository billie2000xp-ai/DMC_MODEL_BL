...
bool MemorySystem::can_merge_write_pair(const Transaction *first, const Transaction *second) const {
    ...
    if (first->task != second->task) return false;
...
bool MemorySystem::dispatch_buffered_write(size_t entry_index, bool merged_dispatch) {
    ...
    bool ret = false;
    if (merged_dispatch) {
        uint64_t external_task = entry.first_trans->task;
        unsigned upstream_channel = entry.first_trans->channel;
        uint64_t internal_task = allocate_internal_write_task();
        Transaction *merged = build_merged_transaction(entry, internal_task);
        ...
        route.internal_task = internal_task;
        route.remaining_beats = merged->burst_length + 1;
        write_data_routes[external_task].push_back(route);

        InternalWriteRespContext ctx;
        ctx.external_task = external_task;
        ctx.upstream_channel = upstream_channel;
        internal_write_resp_contexts[internal_task] = ctx;

        enqueue_pending_write_resp(external_task, upstream_channel);
...
bool MemorySystem::addTransaction(Transaction *trans) {
    ...
    if (is_write_merge_candidate(trans)) {
        ...
        for (size_t i = 0; i < write_merge_buffer.size(); i++) {
            ...
            if (write_merge_buffer[i].first_trans->task != trans->task) continue;
            ...
            if (!dispatch_buffered_write(i, false)) {
                return false;
            }
            break;
        }
...
//    }
    ...
    if (route_it != write_data_routes.end() && !route_it->second.empty()) {
        ...
        if (DEBUG_BUS) {
            PRINTN(setw(10)<<now()<<" -- WDATA :: BP Wdata, waiting merge dispatch, task="<<taskId<<endl);
        }
        return false;
    }
