...
void LPMemorySystemTop::update() {
    ...
    if (DMC_RW_SYNC_EN && NUM_CHANS >= 2) {
        bool global_block_new_rw = false; 
        for (size_t i = 0; i < NUM_CHANS; i++) {
            if (channels[i]->memoryController->HasRwSyncOldActive()) {
                global_block_new_rw = true;
                break; // 只要有一个通道没排空，全系统都要 block 新方向的数据 
            }
        }

        channels[0]->memoryController->SetRwSyncHint(false, NO_GROUP, global_block_new_rw);
        channels[0]->update();
