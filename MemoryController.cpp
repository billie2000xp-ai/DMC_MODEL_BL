...
void MemoryController::ReturnData_statistics(uint64_t task, uint64_t timeAdded, unsigned qos,
    ...
    if (wb_flag) {
        ...
        // 内部的读命令不应该计入正常的业务读延迟统计中，直接 return
        return; 
    }
...
void MemoryController::update_rwgroup_state() {
    ...
    bool local_need_freeze = false;

    if (!local_need_freeze) {
        // 正常滚动
        ...
        if (DEBUG_BUS) {
            PRINTN(setw(10)<<now()<<" -- SYNC :: Pipeline Frozen globally at the edge!"<<endl);
        }
    }
...
void MemoryController::SetRwSyncHint(bool valid, uint8_t target_group, bool block_new_rw) {
    rw_sync_hint_valid = valid;
    rw_sync_hint_group = target_group;
    rw_sync_block_new_rw = block_new_rw;
...
bool MemoryController::IsRwSyncSoftReady() {
    if (rw_group_state[0] == NO_GROUP || rw_group_state.back() == NO_GROUP) return true;
    if (rw_group_state[0] != rw_group_state.back()) return false;
    if (rw_group_state[0] == READ_GROUP && !in_write_group) return true;
    if (rw_group_state[0] == WRITE_GROUP && in_write_group) return true;

    bool old_is_read = rw_group_state[0] == WRITE_GROUP;
    bool old_cas_ready = false;
    bool new_cas_ready = false;
    for (auto &trans : transactionQueue) {
        if (trans->timeout || trans->issue_size >= trans->data_size) continue;
        bool row_open = bankStates[trans->bankIndex].state->currentBankState == RowActive &&
                bankStates[trans->bankIndex].state->openRowAddress == trans->row;
        bool cas_ready = row_open && (trans->nextCmd >= WRITE_CMD && trans->nextCmd <= READ_P_CMD);
        if (!cas_ready) continue;
        bool is_read = trans->transactionType == DATA_READ;
        if (is_read == old_is_read) old_cas_ready = true;
        else new_cas_ready = true;
        if (old_cas_ready && new_cas_ready) break;
    }

    if (!old_cas_ready) return true;
    if (new_cas_ready) return true;
    if (rw_group_state[0] == READ_GROUP) return rwgrp_ch_cmd_cnt >= RW_GRPCHG_W2R_TH;
    if (rw_group_state[0] == WRITE_GROUP) return rwgrp_ch_cmd_cnt >= RW_GRPCHG_R2W_TH;
    return true;
}

bool MemoryController::IsRwSyncStable() const {
    if (rw_group_state[0] == NO_GROUP || rw_group_state.back() == NO_GROUP) return true;
    if (rw_group_state[0] != rw_group_state.back()) return false;
    if (rw_group_state[0] == READ_GROUP) return !in_write_group;
    if (rw_group_state[0] == WRITE_GROUP) return in_write_group;
    return true;
}

void MemoryController::SetRwSyncAllowNewCas(bool allow) {
    rw_sync_allow_new_cas = allow;
}

bool MemoryController::HasRwSyncOldActive() {
    if (rw_group_state[0] == NO_GROUP || rw_group_state.back() == NO_GROUP) return false;
    if (rw_group_state[0] == rw_group_state.back()) return false;
    for (auto &trans : transactionQueue) {
        if (trans->timeout) continue;
        bool old_read = rw_group_state[0] == READ_GROUP && trans->transactionType == DATA_READ;
        bool old_write = rw_group_state[0] == WRITE_GROUP && trans->transactionType != DATA_READ;
        if (!old_read && !old_write) continue;
        if (trans->nextCmd == activate_cmd && trans->issue_size == 0) {
            continue; 
        }
        if ((trans->act_executing || trans->has_send_act || trans->issue_size > 0) &&
                trans->issue_size < trans->data_size) {
            bool row_open = bankStates[trans->bankIndex].state->currentBankState == RowActive &&
                    bankStates[trans->bankIndex].state->openRowAddress == trans->row;
            bool cas_ready = row_open && (trans->nextCmd >= WRITE_CMD && trans->nextCmd <= READ_P_CMD);
            if (!trans->act_executing && trans->issue_size == 0 && !cas_ready) continue;
            if (DEBUG_BUS) {
                PRINTN(setw(10)<<now()<<" -- SYNC :: old active remain task="<<trans->task<<", type="<<trans->transactionType
                        <<", nextCmd="<<trans->nextCmd<<", issue_size="<<trans->issue_size<<", data_size="<<trans->data_size
                        <<", has_send_act="<<trans->has_send_act<<", act_executing="<<trans->act_executing<<endl);
            }
            return true;
        }
    }
...
void MemoryController::lc(Transaction *t) {
    ...
    if (rw_group_state[0] != NO_GROUP && !t->timeout && t->issue_size == 0 && t->nextCmd != PRECHARGE_PB_CMD
            && !t->act_executing) {
        if (DMC_RW_SYNC_EN) {
            bool is_old_dir_cmd = (rw_group_state[0] == READ_GROUP && t->transactionType == DATA_READ) ||
                    (rw_group_state[0] == WRITE_GROUP && t->transactionType != DATA_READ);
            bool is_new_dir_cas = !is_old_dir_cmd && t->nextCmd != activate_cmd;

            if (rw_sync_block_new_rw && rw_group_state[0] != rw_group_state.back()) {
                if (is_old_dir_cmd && t->nextCmd == activate_cmd) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Drain backpress old dir ACT. task="<<t->task<<endl);
                    return;
                }
                if (is_new_dir_cas) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Drain backpress new dir CAS. task="<<t->task<<endl);
                    return;
                }
            } else {
                if (!rw_sync_allow_new_cas && is_new_dir_cas) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Soft barrier backpress new dir CAS. task="<<t->task<<endl);
                    return;
                }
                if (rw_group_state[0] == READ_GROUP && t->transactionType != DATA_READ && (!LQOS_BP_EN || (lqos_rd_bp && LQOS_BP_EN))) {
                    if (t->nextCmd == activate_cmd || rwgrp_ch_cmd_cnt >= RW_GRPCHG_W2R_TH || !in_write_group) {
                        if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Read group backpress a write command. task="<<t->task<<", nextCmd="<<t->nextCmd<<endl);
                        return;
                    }
                    ...
                    if ((t->nextCmd == READ_CMD || t->nextCmd == READ_P_CMD) && !rank_cmd_high_qos[t->rank] &&
                        ...
                        if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Read group backpress a rank command by highqos. task="<<t->task<<", nextCmd="<<t->nextCmd<<endl);
                        return;
                    }
                    ...
                    if (t->nextCmd == activate_cmd || rwgrp_ch_cmd_cnt >= RW_GRPCHG_R2W_TH || in_write_group) {
                        if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Write group backpress a read command. task="<<t->task<<", nextCmd="<<t->nextCmd<<endl);
                        return;
                    }
                }
            }
            ...
            if (rw_group_state[0] == READ_GROUP && t->transactionType != DATA_READ && (!LQOS_BP_EN || (lqos_rd_bp && LQOS_BP_EN))) {
                if (t->nextCmd == activate_cmd || rwgrp_ch_cmd_cnt >= RW_GRPCHG_W2R_TH || !in_write_group) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: Read group backpress a write command. task="<<t->task<<", nextCmd="<<t->nextCmd<<endl);
                    return;
                }
            } else if (rw_group_state[0] == READ_GROUP && t->transactionType == DATA_READ && RCMD_HQOS_RANK_SWITCH_EN) {
                if ((t->nextCmd == READ_CMD || t->nextCmd == READ_P_CMD) && !rank_cmd_high_qos[t->rank] &&
                        ((unsigned)accumulate(rank_cmd_high_qos.begin(), rank_cmd_high_qos.end(), 0) >= 1)) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: Read group backpress a rank command by highqos. task="<<t->task<<", nextCmd="<<t->nextCmd<<endl);
                    return;
                }
            } else if (rw_group_state[0] == WRITE_GROUP && t->transactionType == DATA_READ && (!LQOS_BP_EN || (lqos_wr_bp && LQOS_BP_EN))) {
                if (t->nextCmd == activate_cmd || rwgrp_ch_cmd_cnt >= RW_GRPCHG_R2W_TH || in_write_group) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: Write group backpress a read command. task="<<t->task<<", nextCmd="<<t->nextCmd<<endl);
                    return;
                }
            }
        }
