...
//    rmw = new Rmw(this,channel,DDRSim_log);
    ...
    rw_sync_master_read_cnt = 0;
    rw_sync_master_write_cnt = 0;
    rw_sync_master_qsize = 0;
    serial_cmd_cnt = 0x0;
...
void MemoryController::update_rwgroup_state() {
    ...
    apply_rw_sync_hint(has_read_cmd, has_write_cmd, high_qos_trig);
...
void MemoryController::SetRwSyncHint(bool valid, uint8_t target_group,
        unsigned master_read_cnt, unsigned master_write_cnt, unsigned master_qsize) {
    rw_sync_hint_valid = valid;
    rw_sync_hint_group = target_group;
    rw_sync_master_read_cnt = master_read_cnt;
    rw_sync_master_write_cnt = master_write_cnt;
    rw_sync_master_qsize = master_qsize;
}
...
bool MemoryController::can_follow_rw_sync(uint8_t target_group, bool has_read_cmd,
        bool has_write_cmd, bool high_qos_trig) {
    if (target_group == NO_GROUP) {
        return GetDmcQsize() < EXGRP_LEVEL;
    }

    if (target_group == WRITE_GROUP) {
        if (!has_write_cmd) return false;
        if (high_qos_trig && has_read_cmd) return false;
        if (!has_read_cmd) return true;
        if (que_write_cnt >= CMD_WLEVELH) return true;
        if (que_read_cnt <= RLEVEL_R2W) return true;
        return (rw_sync_master_write_cnt >= rw_sync_master_read_cnt) && (GetDmcQsize() >= ENGRP_LEVEL);
    }

    if (!has_read_cmd) return false;
    if (high_qos_trig) return true;
    if (!has_write_cmd) return true;
    if (que_read_cnt >= CMD_RLEVELL) return true;
    if (que_write_cnt <= CMD_WLEVELL) return true;
    return (rw_sync_master_read_cnt >= rw_sync_master_write_cnt) && (GetDmcQsize() >= ENGRP_LEVEL);
}

void MemoryController::apply_rw_sync_hint(bool has_read_cmd, bool has_write_cmd,
        bool high_qos_trig) {
    if (!DMC_RW_SYNC_EN || !rw_sync_hint_valid) return;
    if (rw_sync_hint_group == rw_group_state.back()) return;
    if (!can_follow_rw_sync(rw_sync_hint_group, has_read_cmd, has_write_cmd, high_qos_trig)) return;

    rw_group_state.push_back(rw_sync_hint_group);
    serial_cmd_cnt = 0x0;
    rwgrp_ch_cmd_cnt = 0x0;

    if (DEBUG_BUS) {
        PRINTN(setw(10)<<now()<<" -- GRP_SYNC :: target="<<+rw_sync_hint_group
                <<", master_rd="<<rw_sync_master_read_cnt<<", master_wr="<<rw_sync_master_write_cnt
                <<", master_q="<<rw_sync_master_qsize<<", local_rd="<<+que_read_cnt
                <<", local_wr="<<+que_write_cnt<<endl);
    }
}
