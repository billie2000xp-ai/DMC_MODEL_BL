...
void MemoryController::update_rwgroup_state() {
    ...
    bool next_state_will_flip = rw_group_state.size() >= 2 &&
        rw_group_state[0] != NO_GROUP &&
        rw_group_state.back() != NO_GROUP &&
        rw_group_state[0] != rw_group_state.back() &&
        rw_group_state[1] == rw_group_state.back();

    bool local_need_freeze = DMC_RW_SYNC_EN && rw_sync_block_new_rw && next_state_will_flip;
...
bool MemoryController::HasRwSyncOldActive() const {
...
void MemoryController::lc(Transaction *t) {
    ...
    if (rw_group_state[0] != NO_GROUP && !t->timeout && t->issue_size == 0 && t->nextCmd != PRECHARGE_PB_CMD
        ...
        if (DMC_RW_SYNC_EN) {
            // ====================================================================
            // 【多通道强同步模式】：追求绝对的时序隔离，防止 DFI 读写交错
            // ====================================================================
            
            // 判定当前命令是否属于“当前物理总线所在的旧方向”
            ...
            bool is_read_col_cmd = t->nextCmd == READ_CMD || t->nextCmd == READ_P_CMD;
            bool is_write_col_cmd = t->nextCmd == WRITE_CMD || t->nextCmd == WRITE_P_CMD ||
                    t->nextCmd == WRITE_MASK_CMD || t->nextCmd == WRITE_MASK_P_CMD;
            bool is_col_cmd = is_read_col_cmd || is_write_col_cmd;

            if (rw_sync_block_new_rw && rw_group_state[0] != rw_group_state.back()) {
                // -------------------------------------------------------------
                // 状态 A：排空期 (Drain Phase) - 意图已变，正在等待旧数据发完
                // -------------------------------------------------------------
                if (is_old_dir_cmd && t->nextCmd == activate_cmd) {
                    // 旧方向：断其后路。禁止再开新行，只准排空已有数据。
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Drain backpress old dir ACT. task="<<t->task<<endl);
                    return;
                }
                if (!is_old_dir_cmd && is_col_cmd) {
                    // 新方向：按兵不动。只准提前开行预热(ACT)，绝对禁止发数据(CAS)上总线！
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Drain backpress new dir CAS. task="<<t->task<<endl);
                    return;
                }
            } else {
                if (!is_old_dir_cmd && is_col_cmd) {
                    if (DEBUG_BUS) PRINTN(setw(10)<<now()<<" -- LC :: SYNC Stable backpress opposite column. task="<<t->task<<endl);
                    return;
                }
            }
        } 
